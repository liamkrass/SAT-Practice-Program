...
const ProfessionalPlinko: React.FC<ProfessionalPlinkoProps> = ({ onClose, balance, onEarn }) => {
  ...
  const [gameState, setGameState] = useState<GameState>(() => ({
    balance: typeof balance === 'number' ? balance : getSavedBalance() || 1000.00,
    betAmount: 1.00,
    riskLevel: 'medium',
    rows: 12,
    playMode: 'manual',
  ...
  useEffect(() => {
    if (typeof balance === 'number') {
      setGameState(prev => ({ ...prev, balance: roundMoney(balance) }));
    }
  }, [balance]);
  ...
  const updateBall = useCallback((ball: Ball): Ball => {
    ...
    if (ball.y > CANVAS_HEIGHT - 120 && !ball.finished) {
      ...
      if (slot) {
        ...
        // Update game state
        setTimeout(() => {
          const netWin = subtractMoney(ball.payout, gameState.betAmount);
          setGameState(prev => ({
            ...prev,
            lastPayout: ball.payout,
            totalWinnings: addMoney(prev.totalWinnings, netWin),
  ...
  }, [gameState.betAmount, gameState.balance, gameState.riskLevel, onEarn]);
